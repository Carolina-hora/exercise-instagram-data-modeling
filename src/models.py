...
class MyUser(Base):
    ...
    followers = relationship('follower', foreign_keys = user_from_id, post_update = True)
...
class Follower(Base):
    ...
    users = relationship('my_user', foreign_keys = user_to_id)
...
class Comment(Base):
    ...
    author_id = Column(Integer, ForeignKey('my_user.id'))
    post_id = Column(Integer, ForeignKey('post.id'))
